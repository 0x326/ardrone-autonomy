...
Controller.prototype._hasNecessaryData = function (navdata) {
    'use strict';
    //TODO: Make better test of navdata
...
Controller.prototype.forward = function(distance, callback) {
    ...
    if (state === null) {
        ...
        var worldX = state.x + Math.cos(state.yaw) * distance;
        var worldY = state.y + Math.sin(state.yaw) * distance;

        // Assign the new goal
        this._go({x: worldX, y: worldY, z: state.z, yaw: state.yaw}, callback);
    }
};
...
Controller.prototype.right = function(distance, callback) {
    ...
    if (state === null) {
        ...
        // Remap our target position in the world coordinates
        var worldX = state.x - Math.sin(state.yaw) * distance;
        var worldY = state.y + Math.cos(state.yaw) * distance;
...
Controller.prototype.cw = function(angle, callback) {
    'use strict';
    var state = this.state();
    var yaw   = toDeg(state.yaw) + angle;

    return this._go({x: state.x, y: state.y, z: state.z, yaw: toRad(yaw)}, callback);
};
...
Controller.prototype._processNavdata = function(navdata) {
    ...
    if (navdata.visionDetect && navdata.visionDetect.nbDetected > 0) {
        ...
        var cameraX = navdata.visionDetect.xc[0],
            cameraY = navdata.visionDetect.yc[0],
            cameraWidth = navdata.visionDetect.width[0],
            cameraHeight = navdata.visionDetect.height[0],
            yaw = navdata.visionDetect.orientationAngle[0],
            distance = navdata.visionDetect.dist[0] / 100 // Need meters
          ;
        ...
        var camera = this._camera.p2m(cameraX + cameraWidth/2, cameraY + cameraHeight/2, distance);
...
Controller.prototype._control = function(navdata) {
    ...
    if (this._goal === null || this._state ===  null) {
        return;
    }
